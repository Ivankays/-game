...
canvas = tk.Canvas(root, width=860, height=540)
canvas.pack()
...
def victory():
    ...
    canvas.create_text(730, 50, text=f"Игрок 1: {len(vr)}", font=("Helvetica", 24), fill="red",  tag = 'store')
    canvas.create_text(730, 90, text=f"Игрок 2: {len(vb)}", font=("Helvetica", 24), fill="blue", tag = 'store')
    if len(vb)==0:
        canvas.delete('all')
        button.destroy()  
    ...
    if len(gg) == 110-1:
        ...
        if len(vr) > len(vb):
            ...
            canvas.create_text(430, 270, text="Ничья", font=("Helvetica", 24), fill="black")
...
def click_handler(event):
    ...
    if step==1 or step==2:
        ...
        if (step==1 or step==2) and 'red' in for_not:
            for_dell=coor 
    ...
    if step==3 or step==4:
        c=1
        for_not=canvas.gettags(canvas.find_closest(x, y)[0])
        if (step==3 or step==4) and 'blue' in for_not:
            for_dell=coor
            canvas.delete('hod3','hod2')
            click(coor,c)
            step=4
